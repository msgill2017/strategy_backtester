...
class TestTrade(TestCase):
    ...
    def test_get_underline_price(self):
        assert get_underline_price(self.fixture_df) == 174.3

    def test_get_strike_price(self):
        expected = [210, 205, 200, 195, 190, 185, 180, 175, 170, 165]

        assert get_strike_price(self.fixture_df) == expected
    ...
    def test_validate_strike_price(self):
        ...
        assert validate_strike_price('210', self.fixture_df) == (True, 210.0)
        assert validate_strike_price('270', self.fixture_df) == (False, 'Valid Strike Price from Available list ')
        assert validate_strike_price('210.0', self.fixture_df) == (True, 210.0)
        assert validate_strike_price('210.5', self.fixture_df) == (False, 'Valid Strike Price from Available list ')
        assert isinstance(validate_strike_price('210', self.fixture_df)[1], float) is True
    ...
    def test_validate_premium(self):
        ...
        assert validate_premium('3', self.fixture_df, trade1) == (True, 3.0)
        assert validate_premium('2.96', self.fixture_df, trade1) == (True, 2.96)
        assert validate_premium('2.94', self.fixture_df, trade1) == (False, ' Premium within range [4.3, 2.95] ')
        assert validate_premium('3', self.fixture_df, trade2) == (True, 3.0)
        assert validate_premium('2.96', self.fixture_df, trade2) == (True, 2.96)
        assert validate_premium('2.89', self.fixture_df, trade2) == (False, ' Premium within range [3.7, 2.9] ')
        assert isinstance(validate_premium('2.96', self.fixture_df, trade2)[1], float) is True
    ...
    def test_premium_range(self):
        ...
        c_h_l = [4.3, 2.95]
        p_h_l = [3.7, 2.9]
        assert all([a == b for a, b in zip(c_h_l, premium_range(self.fixture_df, call, 195))])
        assert all([a == b for a, b in zip(p_h_l, premium_range(self.fixture_df, put, 195))])
        assert isinstance(premium_range(self.fixture_df, put, 195), list) is True
        assert len(premium_range(self.fixture_df, put, 195)) == 2
